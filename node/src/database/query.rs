...
impl DbQuery {
    ...
    pub(crate) fn get_message(&self, request_id:String) -> Result<SignedConsensusMessage> {
        let mut stmt = self.connection.prepare_cached("SELECT request_id, message, signatures FROM messages WHERE request_id = ?1")?;
        let message = stmt.query_row(params![request_id], |row| {
            let request_id: String = row.get(0)?;
            let message: Vec<u8> = row.get(1)?;
            let signatures: Vec<u8> = row.get(2)?;
            let signatures = serde_json::from_slice(&signatures)
                .map_err(|e| {
